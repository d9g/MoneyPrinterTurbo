...
# mv_panel 顶层的 @st.dialog(tr(...)) 装饰器在模块导入时就会求值，所以必须先
# 把 tr 注册进 webui.translation，再导入 mv_panel（这里不能挪到文件顶部）。
from webui import translation

translation.set_translator(tr)

from webui import mv_panel  # noqa: E402
            ...
            with st.container(
                ...
                with action_cols[0]:
                    ...
                    if st.button(
                        ...
                        mv_panel.render_mv_analysis_dialog(task_id, task)
                with action_cols[1]:
                    # 老 0 (P2-6 后变为 1)
                    play_label = tr("Play")
                    if st.button(
...
def _render_settings_dialog():
    with st.container():
        ...
        # === MV 分析设置 () ===
        mv_panel.render_mv_analysis_settings(mv_config_panel)
    ...
    if params.bgm_type == "custom":
        ...
        if uploaded_bgm_file is not None and bgm_enabled:
            try:
                ...
                mv_panel.render_audio_analysis_panel(uploaded_bgm_file, key_prefix="bgm")
        ...
        with st.container(border=True):
            ...
            if tts_mode_enabled:
                ...
                if uploaded_audio_file:
                    ...
                    mv_panel.render_audio_analysis_panel(uploaded_audio_file, key_prefix="voice")
...
def _render_application():
    ...
    mv_panel.apply_pending_mv_audio()
